...
MqttClient.prototype.sendPing = function () {
    if (this.stream.socket.readyState === this.stream.socket.CLOSED || this.stream.socket.readyState === this.stream.socket.CLOSING) {
      this.emit('error', new Error('WebSocket is already in CLOSING or CLOSED state.'));
      return;
    }
    this.stream.socket.send(Buffer.from(['12' << 4, 0]))
}
